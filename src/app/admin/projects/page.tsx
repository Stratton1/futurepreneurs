  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-6">
        <Link
          href="/admin/projects"
          className={`rounded-lg px-3 py-1.5 text-sm font-medium ${!status ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
        >
          All
        </Link>
        <Link
          href="/admin/projects?status=live"
          className={`rounded-lg px-3 py-1.5 text-sm font-medium ${status === 'live' ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
        >
          Live
        </Link>
        <Link
          href="/admin/projects?status=funded"
          className={`rounded-lg px-3 py-1.5 text-sm font-medium ${status === 'funded' ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
        >
          Funded
        </Link>
      </div>
            ...
            <tbody>
              {projects.map((p) => (
                <tr key={p.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="p-3 text-gray-700">{CURRENCY_SYMBOL}{p.total_raised.toFixed(2)}</td>
